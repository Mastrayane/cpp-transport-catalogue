...
namespace graph {
	...
	public:
		...
		std::optional<RouteInfo> FindRoute(VertexId from, VertexId to) const;
	...
	template<typename Weight>
	inline std::optional<typename Router<Weight>::RouteInfo> Router<Weight>::FindRoute(VertexId from, VertexId to) const
	{
		const auto& route_internal_data = routes_internal_data_.at(from).at(to);
		if (!route_internal_data) {
			return std::nullopt;
